import { MODAL_ROUTE_IDS, PAGE_PATHS } from "../../shared/config.js";

export function createNavigationUi({ pages, navLinks, pageMap, mobileNavController }) {
  ...
  const syncFromUrl = () => {
    ...
    const resolved = resolveRouteState({
      hash,
      pathTarget: getTargetFromPathname(),
    });
    ...
    if (resolved.historyTargetId && resolved.historyPath) {
      history.replaceState(
        { type: "page", targetId: resolved.historyTargetId },
        "",
