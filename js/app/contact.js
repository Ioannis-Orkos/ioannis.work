...
function validateForm(formEl) {
  ...
  if (!subject) {
    ...
    showError(formEl, "contact-subject", "Subject must be between 5 and 100 characters.");
    valid = false;
  }
  ...
  if (!message) {
    ...
    showError(formEl, "contact-message", "Message must be between 10 and 1000 characters.");
    valid = false;
  }
...
function showSuccessState(formEl, thankYouEl, statusEl, modalCardEl, modalTitleEl) {
  ...
  if (modalCardEl) modalCardEl.classList.add("contact-success");
  if (modalTitleEl) modalTitleEl.hidden = true;
  if (statusEl) statusEl.hidden = true;
}

function resetFormState(formEl, thankYouEl, submitBtnEl, statusEl, modalCardEl, modalTitleEl) {
  clearSuccessWindow();
  formEl.hidden = false;
  if (thankYouEl) thankYouEl.hidden = true;
  if (modalCardEl) modalCardEl.classList.remove("contact-success");
  if (modalTitleEl) modalTitleEl.hidden = false;
  if (statusEl) statusEl.hidden = false;
  formEl.reset();
  formEl.querySelectorAll("input, textarea, button").forEach((el) => {
    el.disabled = false;
  });
  if (submitBtnEl) submitBtnEl.textContent = "Send Message";
  setStatus(statusEl, "");
...
async function sendEmail({
  ...
  if (typeof window.emailjs === "undefined") {
    setStatus(statusEl, "EmailJS is not loaded. Please check your internet connection.", "red");
    return;
  }
...
export function initContactForm() {
  ...
  const resetContactUi = () => {
    resetFormState(formEl, thankYouEl, submitBtnEl, statusEl, modalCardEl, modalTitleEl);
  };
